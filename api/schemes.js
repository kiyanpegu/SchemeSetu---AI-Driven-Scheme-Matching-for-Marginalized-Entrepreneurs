...
export default function handler(req, res) {
  ...
  try {
      ...
      filtered = filtered.filter(
        (s) =>
          (s.beneficiary_category &&
            s.beneficiary_category.toLowerCase().includes(catLower)) ||
          (s.target_groups &&
            s.target_groups.some((tg) => tg.toLowerCase().includes(catLower))),
      );
    }
    ...
    if (max_income && !isNaN(Number(max_income))) {
      ...
      filtered = filtered.filter(
        (s) =>
          !s.annual_family_income_limit || s.annual_family_income_limit >= inc,
      );
    }
    ...
    if (max_cost && !isNaN(Number(max_cost))) {
      ...
      filtered = filtered.filter(
        (s) => !s.project_cost_max || s.project_cost_max >= cost,
      );
    }
    ...
    if (type) {
      ...
      filtered = filtered.filter(
        (s) => s.scheme_type && s.scheme_type.toLowerCase().includes(typeLower),
      );
    }
    ...
    if (search) {
      ...
      filtered = filtered.filter(
        (s) =>
          ...
          (s.eligible_activities &&
            s.eligible_activities.some((a) => a.toLowerCase().includes(q))),
      );
    }
    ...
    return res
      ...
      .json({
        success: false,
        error: "Internal server error processing schemes request",
      });
  }
}
