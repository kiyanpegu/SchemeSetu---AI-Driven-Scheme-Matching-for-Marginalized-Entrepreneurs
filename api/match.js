...
export default function handler(req, res) {
  ...
  try {
    ...
    schemes.forEach(scheme => {
      ...
      if (isMatch) {
        ...
        if (scheme.subsidy_percentage > 0) {
          subsidyAmount = Math.round((userAmt * scheme.subsidy_percentage) / 100);
        }
    ...
    return res.status(200).json({
      success: true,
      query: { caste, income: userInc, loanAmount: userAmt, age: userAge, gender, urbanRural },
    ...
    return res.status(500).json({ success: false, error: 'Internal server error in matching engine' });
  }
}
